...
$scoreboard players set @s aj.animation_smithie_direct_gangsta.frame $(to_frame)
scoreboard players operation #this aj.i = @s aj.tween_duration
tag @s add aj.transforms_only
execute at @s run function animated_java:smithie_direct/animations/animation_smithie_direct_gangsta/zzz/apply_frame {frame: 0}
$execute at @s run function animated_java:smithie_direct/animations/animation_smithie_direct_gangsta/zzz/apply_frame {frame: $(to_frame)}
