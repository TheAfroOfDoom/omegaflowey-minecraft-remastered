execute unless data entity @s item.components."minecraft:custom_data".effects[] run return run function summit.dev:zz/vehicle/tick/effects/done
...
data remove entity @s item.components."minecraft:custom_data".effects[0]
execute if score #done summit.temp matches 1.. run data modify entity @s item.components."minecraft:custom_data".effects set value []
