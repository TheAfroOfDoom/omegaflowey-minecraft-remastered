...
$execute if score @s omegaflowey.attack.bullets.count matches 0 run function omegaflowey:entity/directorial/boss_fight/summit-2026/origin/at/y { \
  command: "execute positioned $(x) ~-4.0 $(z) run function aj:omegaflowey_dentata_snake_ball/summon { \
    args: { animation: 'omegaflowey_roll_bite', start_animation: true, variant: 'default' } \
  }" \
}
# bullet tail
$execute unless score @s omegaflowey.attack.bullets.count matches 0 run function omegaflowey:entity/directorial/boss_fight/summit-2026/origin/at/y { \
  command: "execute positioned $(x) ~-4.0 $(z) run function aj:omegaflowey_dentata_snake_ball/summon { args: { variant: 'tail' } }" \
}
