# if already in bossfight, do nothing
execute if entity @s[tag=omegaflowey.player.fighting_flowey] run return 0
# execute unless entity @s[tag=!omegaflowey.player.in_queue, tag=!omegaflowey.player.fighting_flowey] run return 0

execute store result score #omegaflowey.bossfight.queue_return_code omegaflowey.global.flag run \
  function summit.activity:api/enqueue_player {booth_id: "omegaflowey", activity_id: "bossfight"}

# if already in another activity (-1) or queued for the bossfight (-2), do nothing
execute if score #omegaflowey.bossfight.queue_return_code omegaflowey.global.flag matches -2..-1 run return 0
...
#   function omegaflowey:entity/directorial/boss_fight/summit-2026/player/queue/join/log_players_ahead_of_you
