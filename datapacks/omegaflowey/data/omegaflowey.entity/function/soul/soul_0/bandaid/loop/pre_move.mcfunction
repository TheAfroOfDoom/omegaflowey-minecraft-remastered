...
execute store result storage utils:math.min b int 1 run data get storage omegaflowey:soul.0.bandaid rotation 100
function omegaflowey.utils:math/min
execute store result score @s omegaflowey.math.0 run data get storage utils:math.min out

data merge storage utils:math.max { a: -1000 }
execute store result storage utils:math.max b int 1 run scoreboard players get @s omegaflowey.math.0
function omegaflowey.utils:math/max

execute store result storage omegaflowey:soul.0.bandaid rotation float 0.01 run data get storage utils:math.max out

# TODO(88): validate bandaid XY velocity bounds
# velocity bounds: [0.1, 0.4]
data merge storage utils:math.min { a: 40 }
execute store result storage utils:math.min b int 1 run data get storage omegaflowey:soul.0.bandaid velocity 100
function omegaflowey.utils:math/min
execute store result score @s omegaflowey.math.0 run data get storage utils:math.min out
...
execute store result storage omegaflowey:soul.0.bandaid velocity float 0.01 run data get storage utils:math.max out
