## Runs tick-wise on the `attack-executor` for the `dentata-snakes` attack
...
execute if score @s omegaflowey.math.0 matches 0 run function omegaflowey.entity:hostile/omega-flowey/attack/dentata-snakes/executor/loop/summon_indicator

# Terminate
execute if score @s omegaflowey.attack.clock.i = @s omegaflowey.attack.executor.clock.length run \
  function omegaflowey.entity:hostile/omega-flowey/attack/dentata-snakes/executor/terminate with storage omegaflowey:bossfight
