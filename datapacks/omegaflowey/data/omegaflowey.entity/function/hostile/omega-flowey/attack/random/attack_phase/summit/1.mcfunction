scoreboard players set #omegaflowey.attack.bomb omegaflowey.attack.weight 5
scoreboard players set #omegaflowey.attack.dentata-snakes omegaflowey.attack.weight 5
scoreboard players set #omegaflowey.attack.finger-guns omegaflowey.attack.weight 5
scoreboard players set #omegaflowey.attack.homing-vines omegaflowey.attack.weight 5

# Run base `start` function
function omegaflowey:entity/hostile/omega-flowey/attack/random/start
