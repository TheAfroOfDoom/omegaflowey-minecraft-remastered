...
scoreboard players operation @s omegaflowey.attack.position.z += @s math.0

# Bound `omegaflowey.attack.position.z` within arena
# lower-bound
scoreboard players set @s math.0 -3050
scoreboard players operation @s math.0 += #omega-flowey.bossfight.summit.origin.z omegaflowey.global.flag
execute store result storage utils:math.max a int 1 run scoreboard players get @s math.0
