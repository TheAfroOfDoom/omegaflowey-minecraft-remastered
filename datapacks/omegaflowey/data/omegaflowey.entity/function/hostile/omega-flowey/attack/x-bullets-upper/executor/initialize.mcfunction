...
scoreboard players set #attack.x-bullets-upper.upper_eye_exists omegaflowey.attack.flag 1
$execute \
  unless entity $(attack_upper_eye_right_uuid) \
  unless entity $(attack_upper_eye_left_uuid) \
  run scoreboard players set #attack.x-bullets-upper.upper_eye_exists omegaflowey.attack.flag 0

# throw an error if no model exists
execute if score #attack.x-bullets-upper.upper_eye_exists omegaflowey.attack.flag matches 0 run function omegaflowey.entity:hostile/omega-flowey/attack/x-bullets-shared/executor/initialize/error { "type": "upper" }
execute if score #attack.x-bullets-upper.upper_eye_exists omegaflowey.attack.flag matches 0 run return fail
...
# Set scores
scoreboard players operation @s omegaflowey.attack.clock.i -= #attack-x-bullets-upper omegaflowey.attack.executor.clock.delay
