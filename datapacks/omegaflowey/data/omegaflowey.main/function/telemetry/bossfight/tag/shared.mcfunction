...
execute store result storage omegaflowey:telemetry tags.bossfight[-1][-1].tick int 1 run scoreboard players get @s omegaflowey.math.0

execute if data storage omegaflowey:telemetry.temp data run \
  data modify storage omegaflowey:telemetry tags.bossfight[-1][-1].data set from storage omegaflowey:telemetry.temp data
