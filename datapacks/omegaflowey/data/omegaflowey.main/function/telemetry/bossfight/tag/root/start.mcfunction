...
data modify storage omegaflowey:telemetry data.bossfight append value [{ name: "root", type: "start" }]

execute store result score #omegaflowey.telemetry.bossfight.basetime omegaflowey.global.flag run time query gametime
execute store result storage omegaflowey:telemetry data.bossfight[-1][0].tick int 1 run \
...
data modify storage omegaflowey:telemetry data.bossfight[-1][-1].player_id set from storage omegaflowey:bossfight active_player_uuid
...
data modify storage omegaflowey:telemetry data.bossfight[-1][-1].player_gamemode set from storage omegaflowey:telemetry.temp data.player_gamemode
