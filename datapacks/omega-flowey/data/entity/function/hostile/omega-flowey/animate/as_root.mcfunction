...
execute \
  unless score #omegaflowey.bossfight.skip_reanimate_largesidevine global.flag matches 1 \
  if entity @s[tag=aj.large_side_vine.root] \
  run function entity:hostile/omega-flowey/animate/large_side_vine
...
execute if entity @s[tag=aj.upper_eye.root] run function animated_java:upper_eye/animations/look_around/resume
